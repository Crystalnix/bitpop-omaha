...
TEST(OmahaCustomizationTest, Constants_BuildFiles) {
  ...
  EXPECT_STREQ("holupdate", MAIN_DLL_BASE_NAME_ANSI);
  ...
  EXPECT_STREQ(_T("npHouseOfLifeOneClick"), ONECLICK_PLUGIN_NAME);
  EXPECT_STREQ(_T("npHouseOfLifeUpdate"), UPDATE_PLUGIN_NAME);
  EXPECT_STREQ(_T("HolupdateBho"), BHO_NAME);

  // Filenames from omaha_version_utils.
  EXPECT_STREQ(
      _T("npHouseOfLifeOneClick") _T(ONECLICK_PLUGIN_VERSION_ANSI) _T(".dll"),
      ONECLICK_PLUGIN_FILENAME);
  EXPECT_STREQ(_T("npHouseOfLifeUpdate") _T(UPDATE_PLUGIN_VERSION_ANSI) _T(".dll"),
               UPDATE_PLUGIN_FILENAME);
  EXPECT_STREQ(_T("HolupdateBho.dll"), BHO_FILENAME);
}
...
TEST(OmahaCustomizationTest, Constants_Names) {
  ...
  // Filename bases
  EXPECT_STREQ(_T("BitpopUpdate"), MAIN_EXE_BASE_NAME);
  EXPECT_STREQ(_T("holupdate"), MAIN_DLL_BASE_NAME);
}

TEST(OmahaCustomizationTest, Constants_Filenames) {
  EXPECT_STREQ(_T("BitpopUpdate.exe"), kOmahaShellFileName);
  EXPECT_STREQ(_T("HouseOfLifeCrashHandler.exe"), kCrashHandlerFileName);
  EXPECT_STREQ(_T("holupdate.dll"), kOmahaDllName);
  EXPECT_STREQ(_T("holupdateres_%s.dll"), kOmahaResourceDllNameFormat);
  EXPECT_STREQ(_T("HouseOfLifeUpdateBroker.exe"), kOmahaBrokerFileName);
  EXPECT_STREQ(_T("HouseOfLifeUpdateOnDemand.exe"), kOmahaOnDemandFileName);
...
TEST(OmahaCustomizationTest, Constants_Certificate) {
  EXPECT_STREQ(_T("House of Life"), kCertificateSubjectName);
}

TEST(OmahaCustomizationTest, Constants_OmahaAppId_String) {
  EXPECT_STREQ(_T("{32E4419B-847F-4870-8640-073EF02C1890}"), GOOPDATE_APP_ID);
  EXPECT_STREQ(_T("{32E4419B-847F-4870-8640-073EF02C1890}"),
               kGoogleUpdateAppId);
}

TEST(OmahaCustomizationTest, Constants_OmahaAppId_GUID) {
  const GUID kExpectedGoogleUpdateGuid =
      {0x32E4419B, 0x847F, 0x4870,
...
TEST(OmahaCustomizationTest, Constants_Directories) {
  ...
  EXPECT_STREQ(_T("HouseOfLife"), OMAHA_REL_COMPANY_DIR);
  EXPECT_STREQ(_T("HouseOfLife\\CrashReports"), OMAHA_REL_CRASH_DIR);
  EXPECT_STREQ(_T("HouseOfLife\\BitpopUpdate"), OMAHA_REL_GOOPDATE_INSTALL_DIR);
  EXPECT_STREQ(_T("HouseOfLife\\BitpopUpdate\\Log"), OMAHA_REL_LOG_DIR);
  EXPECT_STREQ(_T("HouseOfLife\\BitpopUpdate\\Offline"),
                  OMAHA_REL_OFFLINE_STORAGE_DIR);
  EXPECT_STREQ(_T("HouseOfLife\\BitpopUpdate\\Download"),
                  OMAHA_REL_DOWNLOAD_STORAGE_DIR);
  EXPECT_STREQ(_T("HouseOfLife\\BitpopUpdate\\Install"),
...
TEST(OmahaCustomizationTest, Constants_RegistryKeys) {
  EXPECT_STREQ(_T("Software\\HouseOfLife\\"), COMPANY_MAIN_KEY);
  EXPECT_STREQ(_T("Software\\HouseOfLife\\BitpopUpdate\\"), GOOPDATE_MAIN_KEY);
  EXPECT_STREQ(_T("Software\\HouseOfLife\\BitpopUpdate\\Clients\\"), GOOPDATE_REG_RELATIVE_CLIENTS);  // NOLINT
  EXPECT_STREQ(_T("Software\\HouseOfLife\\BitpopUpdate\\ClientState\\"), GOOPDATE_REG_RELATIVE_CLIENT_STATE);  // NOLINT
  EXPECT_STREQ(_T("Software\\HouseOfLife\\BitpopUpdate\\ClientStateMedium\\"), GOOPDATE_REG_RELATIVE_CLIENT_STATE_MEDIUM);  // NOLINT
  EXPECT_STREQ(_T("Software\\Policies\\HouseOfLife\\"), COMPANY_POLICIES_MAIN_KEY);
  EXPECT_STREQ(_T("Software\\Policies\\HouseOfLife\\BitpopUpdate\\"), GOOPDATE_POLICIES_RELATIVE);  // NOLINT

  EXPECT_STREQ(_T("HKCU\\Software\\HouseOfLife\\"), USER_REG_GOOGLE);
  EXPECT_STREQ(_T("HKCU\\Software\\HouseOfLife\\BitpopUpdate\\"), USER_REG_UPDATE);
  EXPECT_STREQ(_T("HKCU\\Software\\HouseOfLife\\BitpopUpdate\\Clients\\"), USER_REG_CLIENTS);  // NOLINT
  EXPECT_STREQ(_T("HKCU\\Software\\HouseOfLife\\BitpopUpdate\\Clients\\{32E4419B-847F-4870-8640-073EF02C1890}"), USER_REG_CLIENTS_GOOPDATE);  // NOLINT
  EXPECT_STREQ(_T("HKCU\\Software\\HouseOfLife\\BitpopUpdate\\ClientState\\"), USER_REG_CLIENT_STATE);  // NOLINT
  EXPECT_STREQ(_T("HKCU\\Software\\HouseOfLife\\BitpopUpdate\\ClientState\\{32E4419B-847F-4870-8640-073EF02C1890}"), USER_REG_CLIENT_STATE_GOOPDATE);  // NOLINT

  EXPECT_STREQ(_T("HKLM\\Software\\HouseOfLife\\"), MACHINE_REG_GOOGLE);
  EXPECT_STREQ(_T("HKLM\\Software\\HouseOfLife\\BitpopUpdate\\"), MACHINE_REG_UPDATE);
  EXPECT_STREQ(_T("HKLM\\Software\\HouseOfLife\\BitpopUpdate\\Clients\\"), MACHINE_REG_CLIENTS);  // NOLINT
  EXPECT_STREQ(_T("HKLM\\Software\\HouseOfLife\\BitpopUpdate\\Clients\\{32E4419B-847F-4870-8640-073EF02C1890}"), MACHINE_REG_CLIENTS_GOOPDATE);  // NOLINT
  EXPECT_STREQ(_T("HKLM\\Software\\HouseOfLife\\BitpopUpdate\\ClientState\\"), MACHINE_REG_CLIENT_STATE);  // NOLINT
  EXPECT_STREQ(_T("HKLM\\Software\\HouseOfLife\\BitpopUpdate\\ClientState\\{32E4419B-847F-4870-8640-073EF02C1890}"), MACHINE_REG_CLIENT_STATE_GOOPDATE);  // NOLINT
  EXPECT_STREQ(_T("HKLM\\Software\\HouseOfLife\\BitpopUpdate\\ClientStateMedium\\"), MACHINE_REG_CLIENT_STATE_MEDIUM);  // NOLINT

  EXPECT_STREQ(_T("HKLM\\Software\\HouseOfLife\\BitpopUpdateDev\\"), MACHINE_REG_UPDATE_DEV);  // NOLINT
}

TEST(OmahaCustomizationTest, Constants_RegistryKeys_GroupPolicy) {
  EXPECT_STREQ(_T("Software\\Policies\\HouseOfLife\\BitpopUpdate\\"), GOOPDATE_POLICIES_RELATIVE);  // NOLINT
  EXPECT_STREQ(_T("HKLM\\Software\\Policies\\HouseOfLife\\BitpopUpdate\\"), kRegKeyGoopdateGroupPolicy);  // NOLINT
}

TEST(OmahaCustomizationTest, Constants_RegistryValues) {
  EXPECT_GU_STREQ(_T("House of Life Update"), kRunValueName);
}

TEST(OmahaCustomizationTest, Constants_MsiMsp) {
  EXPECT_STREQ(_T("HouseOfLifeUpdateHelper.msi"), kHelperInstallerName);
  EXPECT_STREQ(_T("{B29A414C-43FE-40EF-8FC1-141C70C1503B}"),
               kHelperInstallerProductGuid);
  EXPECT_STREQ(_T("HouseOfLifeUpdateHelperPatch.msp"), kHelperPatchName);
  EXPECT_STREQ(_T("{C7DBD047-AE4B-483F-B51E-95605BB1A32C}"), kHelperPatchGuid);
}
...
TEST(OmahaCustomizationTest, Constants_Addresses) {
  EXPECT_STREQ(_T("www.bitpop.com"), kGoogleHttpServer);
  EXPECT_STREQ(_T("tools.bitpop.com"), kGoopdateServer);
  EXPECT_STREQ(_T("https://tools.bitpop.com/service/update2"), kUrlUpdateCheck);
  EXPECT_STREQ(_T("http://tools.bitpop.com/service/update2"), kUrlPing);
  EXPECT_STREQ(_T("http://clients2.bitpop.com/cr/report"), kUrlCrashReport);
  EXPECT_STREQ(_T("http://www.bitpop.com/support/installer/?"), kUrlMoreInfo);
  EXPECT_STREQ(_T("http://cr-tools.clients.bitpop.com/service/check2"),
               kUrlCodeRedCheck);
  EXPECT_STREQ(_T("http://clients5.bitpop.com/tbproxy/usagestats"),
               kUrlUsageStatsReport);
}

TEST(OmahaCustomizationTest, Constants_Config) {
  EXPECT_STREQ(_T("Software\\HouseOfLife\\BitpopUpdate\\Shared"), kCiRegKeyShared);
}

TEST(OmahaCustomizationTest, Constants_Debug) {
  EXPECT_GU_STREQ(_T("BitpopUpdate-debug"), kCiDebugDirectory);
}

TEST(OmahaCustomizationTest, Constants_Logging) {
  EXPECT_STREQ(_T("BitpopUpdate.ini"), kLogConfigFileName);
  EXPECT_STREQ(_T("BitpopUpdate.log"), kDefaultLogFileName);
}
...
TEST(OmahaCustomizationTest, Constants_ObjectNames_MutexesAndEvents) {
  ...
  EXPECT_GU_STREQ(_T("{C68009EA-1163-4498-8E93-D5C4E317D8CE}"),
                  kMetricsSerializer);
  EXPECT_GU_STREQ(_T("{0E900C7B-04B0-47f9-81B0-F8D94F2DF01B}"),
                  kNetworkConfigLock);
...
TEST(OmahaCustomizationTest, Constants_ObjectNames_SharedMemory) {
  EXPECT_STREQ(_T("Global\\HouseOfLifeBitpopUpdate3"),
                  kGoogleUpdate3SharedMemoryName);
  EXPECT_STREQ(_T("Global\\HouseOfLifeBitpopUpdateCore"),
                  kGoogleUpdateCoreSharedMemoryName);
}

TEST(OmahaCustomizationTest, Constants_Services) {
  EXPECT_STREQ(_T("holupdate_service_name"), kRegValueServiceName);
  EXPECT_STREQ(_T("holupdatem_service_name"), kRegValueMediumServiceName);
  EXPECT_STREQ(_T("holupdate_task_name_c"), kRegValueTaskNameC);
  EXPECT_STREQ(_T("holupdate_task_name_ua"), kRegValueTaskNameUA);

  EXPECT_STREQ(_T("holupdate"), kServicePrefix);
  EXPECT_STREQ(_T("holupdatem"), kMediumServicePrefix);

  EXPECT_STREQ(_T("BitpopUpdate.exe"), kServiceFileName);
}
...
TEST(OmahaCustomizationTest, Constants_Plugins) {
  EXPECT_STREQ(_T("HouseOfLife.OneClickCtrl.") _T(ONECLICK_PLUGIN_VERSION_ANSI),
                  kOneClickProgId);
  EXPECT_STREQ(
      "application/x-vnd.bitpop.oneclickctrl." ONECLICK_PLUGIN_VERSION_ANSI,
