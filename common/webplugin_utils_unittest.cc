...
TEST(WebPluginUtilsTest, CopyGoopdateToTempDir) {
  ...
  EXPECT_TRUE(files_map.find(_T("BitpopUpdate.exe")) != files_map.end());
  EXPECT_TRUE(files_map.find(_T("holupdate.dll")) != files_map.end());
  EXPECT_TRUE(files_map.find(_T("holupdateres_en.dll")) != files_map.end());
